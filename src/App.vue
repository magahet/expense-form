<template>
  <div id="app">
    ...
    <a
      target="_blank"
      href="https://docs.google.com/spreadsheets/d/1gfYVFecPnYSJLkVVCIoQEUiKE6P6nSUvra8uxa3FB90/edit?usp=sharing"
    >Click here to see the expenses spreadsheet</a>
    <b-form @submit.prevent="onSubmit" @reset.prevent="onReset" v-if="show">
      ...
      <b-form-checkbox
        class="my-4"
        v-model="itemize"
        @change="toggleItemize"
      >Itemize Expenses (multiple expenses on one receipt)</b-form-checkbox>
      ...
      <b-container class="my-4" fluid>
        <b-form-row v-if="itemize">
          ...
          <b-col cols="2">Tax Rate: {{ toPercent(taxRate) }}</b-col>
        </b-form-row>
        ...
        <b-form-row class="my-4" v-for="(exp, idx) in expenses" :key="idx">
          <Expense
            :itemize="itemize"
            :taxRate="taxRate"
            v-model="expenses[idx]"
            :isFirst="idx == 0"
            :categories="optionGroups"
            @remove="removeExpense(idx)"
          />
        </b-form-row>
        ...
        <b-form-row class="my-4" v-if="itemize">
          ...
          <b-col cols="2">
            <strong>{{ toCurrency(preTaxTotal) }}</strong>
          </b-col>
          <b-col cols="1">
            <strong>{{ toCurrency(postTaxTotal) }}</strong>
          </b-col>
        </b-form-row>
